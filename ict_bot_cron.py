#!/usr/bin/env python3
"""
ICT 5m Signal Bot – Cron-Variante fuer GitHub Actions.
Fuehrt EINEN Durchlauf aus (fetch -> analysieren -> Telegram senden -> Zustand
speichern) und beendet sich. Wird von .github/workflows/ict-bot.yml alle 5
Minuten automatisch gestartet. Kein eigener Server noetig.
"""
...
log = logging.getLogger("ict_bot_cron")
...
def find_swings(candles, n=SWING_N):
    ...
    for i in range(n, len(candles) - n):
        ...
        if is_high:
            highs.append({"idx": i, "price": candles[i]["high"], "time": candles[i]["openTime"]})
        if is_low:
            lows.append({"idx": i, "price": candles[i]["low"], "time": candles[i]["openTime"]})
    return highs, lows
...
def make_setup(symbol, direction, setup_type, sl_basis, entry, opposing_prices, zone=None):
        ...
        if current["close"] > sweep_low:
            ...
            if fvg and fvg["low"] <= current["close"] <= fvg["high"]:
                s = make_setup(symbol, "LONG", "FVG", sweep_low, current["close"], opposing,
                                zone={"low": fvg["low"], "high": fvg["high"]})
                if s:
                    s["id"] = f"{symbol}-LONG-FVG-{c['openTime']}"
                    s["entryTime"] = current["openTime"]
                    results.append(s)
            ...
            if ob and ob["low"] <= current["close"] <= ob["high"]:
                s = make_setup(symbol, "LONG", "OB", ob["low"], current["close"], opposing,
                                zone={"low": ob["low"], "high": ob["high"]})
                if s:
                    s["id"] = f"{symbol}-LONG-OB-{c['openTime']}"
                    s["entryTime"] = current["openTime"]
                    results.append(s)
            ...
            if rng > 0 and ote_low <= current["close"] <= ote_high:
                s = make_setup(symbol, "LONG", "OTE", sweep_low, current["close"], opposing,
                                zone={"low": ote_low, "high": ote_high})
                if s:
                    s["id"] = f"{symbol}-LONG-OTE-{c['openTime']}"
                    s["entryTime"] = current["openTime"]
                    results.append(s)
        ...
        if current["close"] < sweep_high:
            ...
            if fvg and fvg["low"] <= current["close"] <= fvg["high"]:
                s = make_setup(symbol, "SHORT", "FVG", sweep_high, current["close"], opposing,
                                zone={"low": fvg["low"], "high": fvg["high"]})
                if s:
                    s["id"] = f"{symbol}-SHORT-FVG-{c['openTime']}"
                    s["entryTime"] = current["openTime"]
                    results.append(s)
            ...
            if ob and ob["low"] <= current["close"] <= ob["high"]:
                s = make_setup(symbol, "SHORT", "OB", ob["high"], current["close"], opposing,
                                zone={"low": ob["low"], "high": ob["high"]})
                if s:
                    s["id"] = f"{symbol}-SHORT-OB-{c['openTime']}"
                    s["entryTime"] = current["openTime"]
                    results.append(s)
            ...
            if rng > 0 and ote_low <= current["close"] <= ote_high:
                s = make_setup(symbol, "SHORT", "OTE", sweep_high, current["close"], opposing,
                                zone={"low": ote_low, "high": ote_high})
                if s:
                    s["id"] = f"{symbol}-SHORT-OTE-{c['openTime']}"
                    s["entryTime"] = current["openTime"]
                    results.append(s)
        ...
        try:
            ...
            for sig in state["signals"]:
                if sig["symbol"] == symbol and sig["status"] == "OPEN":
                    ...
                    if outcome != "OPEN":
                        sig["status"] = outcome
                        send_discord(f"{sig['direction']} {sig['symbol']} -> {outcome}")
